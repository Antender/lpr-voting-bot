...
def apiCall(method, parameters):
    r = requests_ses.post(f'https://api.telegram.org/bot{TELEGRAM_TOKEN}/{method}', timeout=3, data=parameters)
...
try:
    requests_ses.post(f'https://api.telegram.org/bot{TELEGRAM_TOKEN}/setWebhook', timeout=10, data={
        'url': DOMAIN,
        'max_connections': 1,
